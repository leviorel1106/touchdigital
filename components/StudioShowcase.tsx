...
export function StudioShowcase({ sectionId = "work" }: { sectionId?: string }) {
  ...
  return <section id={sectionId} className="studio-showcase section" aria-labelledby={`${sectionId}-title`} data-scroll-scene>
    <div className="studio-heading"><div><h2 id={`${sectionId}-title`}>אותו דמיון.<br /><span className="heading-accent">עולמות אחרים.</span></h2></div><p>מהמנה הראשונה ועד הבית הבא.<br />לכל עסק יש סיפור שאפשר לראות אחרת.</p></div>
